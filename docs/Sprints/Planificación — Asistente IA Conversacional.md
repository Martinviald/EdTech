...
| **H21.10b** ✅ | **Asistente embebido**: botón flotante + panel lateral (`Sheet`) **con ancho ajustable persistido en localStorage** en el layout del dashboard (gated por rol+feature) + contrato `assistantPageContext` en `@soe/types` + `useRegisterAssistantContext` (registry por vista, reactivo). Cada vista declara sus refs; el panel las envía en `pageContext` por mensaje. **Mapeo de vistas → refs** (ligero, colocado por página): cableado en Informe de Análisis IA (`assessment`+`classGroup`); pendiente replicar en heatmap/dashboards/detalle de alumno (una línea por vista). | Asistente accesible en toda la app con contexto auto-cargado |
