...
| Historia | Descripción | Entregable |
|---|---|---|
| **H21.10** ✅ | Componente de chat con streaming (SSE vía `fetch`+`getReader`), indicador de tool-calls, **render markdown seguro** (react-markdown + remark-gfm, sin HTML crudo), responsive — reusable por el panel. **Streaming sin exponer el token**: POST a un route handler same-origin que adjunta el Bearer de la cookie httpOnly. _Diferido:_ ruta `/asistente` full-page (el panel redimensionable + historial ya cubren "foco/historial"; se retoma si se necesita URL deep-linkable/entrada en el sidebar). | Chat funcional para directivos |
| **H21.10b** ✅ | **Asistente embebido**: botón flotante + panel lateral (`Sheet`) **con ancho ajustable persistido en localStorage** en el layout del dashboard (gated por rol+feature) + contrato `assistantPageContext` en `@soe/types` + `useRegisterAssistantContext` (registry por vista, reactivo). Cada vista declara sus refs; el panel las envía en `pageContext` por mensaje. **Mapeo de vistas → refs** (ligero, colocado por página): cableado en Análisis IA e Informe de evaluación (`assessment`+`classGroup`) y en Mapa de calor + Logro por habilidad (refs derivados de los filtros vía `dashboardFiltersToAssistantRefs`); pendiente replicar en detalle de alumno / progresión / comparación (una línea por vista). | Asistente accesible en toda la app con contexto auto-cargado |
| **H21.11** ✅ | Panel de historial de conversaciones (lista, abrir → carga mensajes en el provider, borrar con soft delete optimista). Alterna chat/historial en el header del panel. | Continuidad de conversaciones |
| **H21.11b** ✅ | **Selector/mención `@` de alumno** (§4.4) como caso del contrato `pageContext` (`kind: 'student'`): endpoint backend `GET /assistant/students?q=` (acotado al org, sin RUT) + autocompletado `@` en el chat (debounce, teclado, chips). Al elegir, inserta el **UUID**; el nombre se queda en el cliente (chip) y nunca viaja al LLM. | Preguntar por un alumno puntual sin enviar PII |
| **H21.12** ✅ | "Pregúntale a la IA sobre esto" — `AskAiButton` reutilizable que abre el panel con un prompt sugerido; usa el `pageContext` que la vista ya declara. Se auto-oculta si el asistente no está habilitado. Cableado en el Informe de Análisis IA. | Punto de entrada contextual |
| **H21.13** ◐ | Tests: unit del service (incl. searchStudents sin RUT) + integración HTTP/SSE del controller + observabilidad. _Pendiente:_ tests de aislamiento multi-tenant vía prompt-injection y guardrail anti-alucinación end-to-end. | Suite de seguridad/calidad |
