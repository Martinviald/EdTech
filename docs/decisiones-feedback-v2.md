...
| ~~**T2-19** Crear material IA~~ | **⏸️ DIFERIDO a fase futura (F2)** — decisión 2026-07-26. No se construye en esta iteración; "Material Remedial" queda como está. El delta original (DTO NL + prompt dinámico + enum + canvas) se retoma más adelante. | — |
...
| **T2-23** Ocultar estados (REDEFINIDO 2026-07-26) | **NO es el `assessment_status` vestigial.** Son los estados **Borrador/Publicado/Archivado** de **instrumentos** (visibles en el banco). Acción: **ocultarlos en la UI** (badges/filtros/selectores) y que **todas las queries traigan todos los estados** (no filtrar por status). **SIN migración** — la columna se conserva. Localizar el enum/campo de status de instrumentos + sus call-sites de UI y de filtro. | UI+Backend-queries |

> **Nota de migraciones:** con **T2-19 diferido** y **T2-23 sin migración** (redefinido), quedan **2 migraciones secuenciales**: **T2-21 → `0015`** (enum `item_difficulty`) y **T2-22 → `0016`** (`item_collections` + `item_collection_items`). Cada una: `pnpm db:generate`, commitear el `.sql`, aplicar/validar en deploy o E2E. **Secuenciales** (conflicto si dos generan el mismo número).
