...
describe('AnswerSheetsService', () => {
  ...
  describe('preview', () => {
    it('devuelve filas con matched/unmatched students', async () => {
      ...
      expect(preview.unmatchedRows).toBe(1);
      // El item de posición 3 no está en el instrumento pero sí en el CSV.
      // El item 1 y 2 del instrumento están en el CSV.
      expect(preview.itemPositions).toEqual(expect.arrayContaining([1, 2, 3]));
    });
