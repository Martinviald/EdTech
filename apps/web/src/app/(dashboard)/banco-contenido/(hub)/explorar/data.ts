...
export const getItems = cache((query: string) => apiGet<ItemsListResponse>(`/items?${query}`));

export const getInstruments = cache(() =>
  apiGet<InstrumentsListResponse>('/instruments?limit=200'),
);

export const getItemCollections = cache(() =>
  apiGet<ItemCollectionListResponse>('/item-collections?limit=100'),
);
